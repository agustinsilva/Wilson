...
//Calculo de tamaño de enjambre basado en dimensionalidad
int pso_calc_swarm_size(int dim) 
{
  int size = 10. + 2. * MathSqrt(dim);
...
//==================================    
//Estrategias de actualizacion de peso de inercia
//Calcula linealmente el decrecimiento del peso de inercia
//====================================
...
// Estrategia de matriz de vecindario
// global neighborhood
      ...
      for (d=0; d<settings->dim; d++) {
        ...
        if (settings->clamp_pos) {
          ...
          if (pos[i][d] < settings->x_lo) {
            ...
            pos[i][d] = settings->x_hi - MathMod(settings->x_lo - pos[i][d],
                                              settings->x_hi - settings->x_lo);
            vel[i][d] = 0;

          } else if (pos[i][d] > settings->x_hi) {

            pos[i][d] = settings->x_lo + MathMod(pos[i][d] - settings->x_hi,
                                              settings->x_hi - settings->x_lo);
            vel[i][d] = 0;
          }
        }
